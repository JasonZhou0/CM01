...
DEVICE = '  -mcpu=cortex-m4 -mthumb -mfpu=fpv4-sp-d16 -mfloat-abi=hard -fsigned-char -ffunction-sections -fdata-sections -std=gnu11 -fmessage-length=0'

BOOT_CompileTool['CCFLAGS']      = DEVICE + ' -g -Wfatal-errors -Wall -Wextra' # -DSTM32F407VE -DSTM32F4XX -DUSE_STDPERIPH_DRIVER -D__ASSEMBLY__ -D__FPU_USED'

BOOT_CompileTool['ASFLAGS']      = ' -c' + DEVICE + ' -x assembler-with-cpp -Wa,-mimplicit-it=thumb '

BOOT_CompileTool['LINKFLAGS']    = DEVICE + ' -lm -lgcc -lc' + ' -nostartfiles -Wl,--gc-sections,-Map=%s.map,-cref,-u,Reset_Handler -T %s'%(BOOT_Out,BOOT_LD)
...
APP_CompileTool['CCFLAGS']      = DEVICE + ' -g -Wfatal-errors -Wall -Wextra' # -DSTM32F407ZG -DSTM32F4XX -DUSE_STDPERIPH_DRIVER -D__ASSEMBLY__ -D__FPU_USED'

APP_CompileTool['ASFLAGS']      = ' -c' + DEVICE + ' -x assembler-with-cpp -Wa,-mimplicit-it=thumb '

APP_CompileTool['LINKFLAGS']    = DEVICE + ' -lm -lgcc -lc' + ' -nostartfiles -Wl,--gc-sections,-Map=%s.map,-cref,-u,Reset_Handler -T %s'%(APP_Out,APP_LD)
