...
uint32_t ETH_Init(ETH_InitTypeDef* ETH_InitStruct, uint16_t PHYAddress)
{
  ...
  if(ETH_InitStruct->ETH_AutoNegotiation != ETH_AutoNegotiation_Disable)
  {
    ...
    do
    {
#if PHY_READ_TO == 0x0004FFFF
      timeout++;
#elif PHY_READ_TO == 0x000001FF
      timeout++;
      delay_ms(10);
#else
#error "请重新设置 PHY_READ_TO 的数值！"
#endif
    } while (!(ETH_ReadPHYRegister(PHYAddress, PHY_BSR) & PHY_Linked_Status) && (timeout < (uint32_t)PHY_READ_TO));
...
uint32_t ETH_WritePHYRegister(uint16_t PHYAddress, uint16_t PHYReg, uint16_t PHYValue)
{
  ...
  do
  {
#if PHY_WRITE_TO == 0x0004FFFF
      timeout++;
#elif PHY_WRITE_TO == 0x000001FF
      timeout++;
      delay_ms(10);
#else
#error "请重新设置 PHY_READ_TO 的数值！"
